...
public protocol PKDatePickerDelegate: class {
    
    func pkDatePicker(_ pkDatePicker: PKDatePicker, didSelect date: Date)
    func pkDatePickerDidCancelSelection(_ pkDatePicker: PKDatePicker)
    
}

public class PKDatePicker: UIView {
    ...
    public weak var delegate: PKDatePickerDelegate?
    ...
    // MARK: - Init
    public static func getFromNib() -> PKDatePicker {
        let podBundle = Bundle(for: PKDatePicker.self)
        let bundleURL = podBundle.url(forResource: "PKDatePicker", withExtension: "bundle")
        let bundle = Bundle(url: bundleURL!)!
        return UINib(nibName: "PKDatePicker", bundle: bundle).instantiate(withOwner: nil, options: nil)[0] as! PKDatePicker
    }
    ...
    @IBAction func confirmButtonDidTapped(_ sender: AnyObject) {
        ...
        dismiss()
        delegate?.pkDatePicker(self, didSelect: datePicker.date)
        
    }
    @IBAction func cancelButtonDidTapped(_ sender: AnyObject) {
        dismiss()
        delegate?.pkDatePickerDidCancelSelection(self)
    }
